...
**Плотность — не на глаз, это планка (иначе прогоны гуляют: тот же ролик выходил то на 39 карточек / 33 цитаты, то на 26 / 16).**
- **Каждая тезис-карточка несёт `«дословную цитату»` из речи** — исключение только у чисто визуального тезиса (показан экран, слова к нему не привязаны). Есть в транскрипте дословная формулировка мысли — цитируй её, **не пересказывай своими словами**: пересказ теряет голос автора и его точные числа/названия.
- **Цель — ~1 карточка на 1,5 минуты содержательного видео** (реклама/вода не в счёт). Час полезного материала → 35–40 карточек, а не 25. Это калибровано по замеру: на одном ролике плотный прогон дал 39 карточек и понравился, ужатый на 26 — нет. Мало карточек = материал утрамбован в пересказ, планка провалена.
- **Числа, названия, ценники — только дословно из речи или с кадра** (`salient`), никогда по памяти. Именно они — самое ценное, что теряется при пересказе.
- Не пересказывай там, где можно процитировать. Пересказ — связка между цитатами, а не замена им.
...
**Структурные слайды видео — обязательные скрины, не «по вкусу».** Есть слайд-оглавление метода («метод из N ступеней», «что будет на эфире») и пронумерованные слайды-заголовки этапов/ступеней — это каркас конспекта. Вставляй их ВСЕ (каркас-слайд + слайд каждой ступени), не заменяй «более интересными» демо: автор посвятил им время, они задают структуру, без них конспект теряет опору. (2026-07-15: заменил каркас-слайд матрицей, а слайды ступеней 2/4 — демо; пользователь указал вернуть — он смотрел эфир и ждал именно их.)

**Критерий проверки строгий, а не «в целом видно».** Открыл (Read) — проверь: заголовок и ключевой текст видны ПОЛНОСТЬЮ; слайд на весь экран, а не в окне браузера (панель вкладок / адресная строка режет верх заголовка); курсор/тултип не поверх текста; вебки в углу не закрывают контент. Обрезано обвязкой браузера → бери другой момент показа: слайд в лекции показывают несколько раз, есть и fullscreen. «Почти видно» — не годится. (2026-07-15: принял кадр слайда «Ступень 3» с панелью Chrome поверх заголовка; пользователь заметил обрезку.)

## Step 6 — render self-contained HTML
For each chosen thesis-screenshot, **re-extract at native resolution** — do NOT reuse the downscaled vision frame (its small text may be unreadable). Pass the representative's **`span_end` seconds** from the Step 2 report (the applied state / fully-rendered slide), not the `t` seconds — **но для СЛАЙДА этого мало, проверь кадр:** `span_end` — последний кадр dedup-диапазона, и если к концу показа слайд свернули в окно/пролистнули, `span_end` ловит уже не слайд (заголовок обрезан панелью браузера), а чистый fullscreen сидит в НАЧАЛЕ диапазона (`t`) или середине. Не совпало — переизвлеки по `t`/середине span и открой (Read) результат. (2026-07-15: `span_end=2091` слайда «Ступень 3» дал кадр в окне браузера, fullscreen был на `t=2038` — начало того же диапазона.)
```bash
...
**Проверь inline по факту, не по коду выхода обёртки.** `inline_images.py` при отсутствии `digest.raw.html` падает `FileNotFoundError` — не гаси его вывод `>/dev/null 2>&1` в связке с `|| echo ok`: так молча останется старый `digest.html`. После инлайна сверь: mtime `digest.html` свежий; `grep -c 'data:image'` = числу скринов; ноль оставшихся `src="screens/..."`. **При правке уже готового конспекта** сначала пересоздай `digest.raw.html` (в Step 7 он удаляется вместе с `screens/` и видео) — иначе переинлайн не из чего делать. Меняешь один скрин — переизвлечь ВСЕ (папку `screens/` тоже удалили), не только новый. (2026-07-15: три «успешных» переинлайна молча не трогали файл — raw был удалён; вскрылось сверкой mtime и размера base64 в самом `digest.html`.)

### Стиль (планка = эталонный конспект)
Self-contained single file. Dark + light theme (system default via `prefers-color-scheme` **plus** a toggle button that stamps `data-theme`). Hero: title, one-line lede, mono meta-chips (длительность, тема, N разделов). Sections of **тезис-карточки** — каждая: заголовок + короткое пояснение + `«дословная цитата»` из речи (обязательна, кроме чисто визуального тезиса; плотность — см. Step 4). `<details>` for secondary «договорённости/цитаты». A **глоссарий** of terms. A **footer disclaimer**: составлено по локальной транскрипции; названия/термины могли быть расслышаны неточно. Screens are illustrations inside the relevant thesis, `max-width:100%`, and **clickable → lightbox**: the thumbnail stays compact in flow, a click opens the shot large over the page (overlay, closes on click/Esc). Так конспект не растягивается, а читаемость экрана доступна по клику. No horizontal page scroll (wide code/tables scroll in their own container).
